...
venn.diagram(x=list("Before"=sediment.before.types.subclade,"Storm"=sediment.storm.types.subclade,"After"=sediment.after.types.subclade),file = "analyses/Sediment_Venn.jpg", col=col[c(1,3,4)],fill=col[c(1,3,4)], margin=0,main = "Symbiodinium Types in Sediment",height=6,width=6,units="in",scaled=T)
...
# draw.triple.venn(length(sediment.before.types.subclade),length(sediment.storm.types.subclade),length(sediment.after.types.subclade),sum(sediment.before.types.subclade %in% sediment.storm.types.subclade), sum(sediment.storm.types.subclade %in% sediment.after.types.subclade),sum(sediment.before.types.subclade %in% sediment.after.types.subclade),sum(sediment.before.types.subclade %in% c(sediment.storm.types.subclade, sediment.after.types.subclade)),file = "analyses/Sediment_Venn.jpg", col=col[c(1,3,4)],fill=col[c(1,3,4)], margin=0,main = "Symbiodinium Types in Sediment",height=6,width=6,units="in",scaled=T)
...
venn.diagram(x=list("Before"=water.before.types.subclade,"Storm"=water.storm.types.subclade,"After"=water.after.types.subclade),file = "analyses/water_Venn.jpg", col=col[c(1,3,4)],fill=col[c(1,3,4)], margin=0,main = "Symbiodinium Types in water",height=6,width=6,units="in",scaled=T)
...
vd1 <- plot(VennDiag, quantities = TRUE, font=1, cex=1, alpha=0.5,
     fill=col,col=col,border=col,lwd=c(1,4,2),auto.key = F,main="Sediment")
vd2 <- plot(VennDiag2, quantities = TRUE, font=1, cex=1, alpha=0.5,
     fill=col,col=col,border=col,lwd=c(1,4,2),labels = c("Before","During","After"),main="Water",key=list(space="right",boxes=list(col=col), lwd=c(1,4,2),text=list(c("Before","During","After"))
     )) # removed: auto.key=T
vd2 <- plot(VennDiag2, quantities = TRUE, font=1, cex=1, alpha=0.5,
            fill=col,col=col,border=col,lwd=c(1,4,2),labels = c("Before","During","After"),main="Water",auto.key = F)
...
venn.diagram(x=list("Coral"=coral.types.subclade,"Sediment"=sediment.types.subclade,"Water"=water.types.subclade),file = "analyses/coral_sediment_water_Venn.jpg", col=cols2,fill=cols2, margin=0,main = "Symbiodinium Types",height=6,width=6,units="in",scaled=T)
...
plot(VennDiag3, quantities = TRUE, font=1, cex=1, alpha=0.5,
     fill=cols2,col=cols2,border=cols2,lwd=c(2,2,2))
dev.off()
